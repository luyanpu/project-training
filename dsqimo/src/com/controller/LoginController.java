...
import com.service.RegistServiceImpl;
...
public class LoginController {
	...
	public String loginByLoginHtml(User u,HttpServletRequest request) {
		//if用户名和密码都正确，且通过登录
		User user=this.loginServiceImpl.loginByNameAndPass(u.getName(),u.getPassword());
		if(user!=null && user.getState()==1) {
			//将这个用户存入session中
			HttpSession session=request.getSession();
			session.setAttribute("user", user);
			return "redirect:/indexshow";//重定向到showController控制器,/代表项目根目录
		}else {
	...
	public String loginByMail(HttpServletRequest request) {
		...
		return "redirect:/userSpace.jsp";//跳转到个人中心页面，添加用户图片信息 代表项目的根目录
	}
	@GetMapping("/c")
